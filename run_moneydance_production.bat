...
set "PS_CMD=$p=Read-Host 'Moneydance password' -AsSecureString; $b=[Runtime.InteropServices.Marshal]::SecureStringToBSTR($p); try {[Runtime.InteropServices.Marshal]::PtrToStringBSTR($b) | & '%MD_JAVA%' '-Djava.awt.headless=true' '-Dpython.cachedir.skip=true' '-cp' '%MD_LIB%' 'org.python.util.jython' '-B' '%MD_RUNNER%' '%MD_CSV_PATH%' '%MD_BOOK_FOLDER%' '%MD_LOG_PATH%'} finally {[Runtime.InteropServices.Marshal]::ZeroFreeBSTR($b)}"
